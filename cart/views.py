    ...
    if request.user.is_authenticated:
        ...
        if is_cartItem_exist:
            ...
            if product_variation in ex_variation_list:
                ...
                get_item = CartItem.objects.get(product=product,id=id)
                get_item.quantity += 1
                get_item.save()
        ...
        is_cartItem_exist=CartItem.objects.filter(product=product,cart=cartId).exists()
        ...
        if is_cartItem_exist:
            ...
            if product_variation in ex_variation_list:
                index=ex_variation.list.index(product_variation)
                id=ex_variation_id[index]
                get_item=CartItem.objects.get(product=product,cart=cartId,id=id)
                get_item.quantity +=1
                get_item.save()

            else:
                cart_item=CartItem.objects.create(product=product,cart=cartId,quantity=1)
                if product_variation is not None:
                    cart_item.variation.add(*product_variation)
                cart_item.save()
...
def checkout(request):
    ...
    try:
        if request.user.is_authenticated:
            cart_item=CartItem.objects.filter(user=request.user,is_available=True)

    except:
        pass

    is_ordered= Order.objects.filter(user=request.user,is_ordered=True).exists()
    if is_ordered:
        form = OrderForm(instance=Order.objects.filter(user=request.user,is_ordered=True))
    else:
        form= OrderForm()
